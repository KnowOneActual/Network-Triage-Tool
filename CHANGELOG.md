...
### Fixed
- **Thread Safety:** Updated `safe_socket_operation` to only use `signal` based timeouts when running in the main thread. This resolves the persistent "Error fetching IP" on the Dashboard which runs in a background thread.
