...
### Added
- **Traffic Health Monitor** — Passive packet sniffer using Scapy to analyze unicast, multicast, and broadcast distributions.
- **Broadcast Protocol Tracker** — Live counters for network broadcast protocols including ARP, DHCP, STP, LLDP, and CDP.
- **History & Baseline Comparison** — Persistent capture history storage with comparative analysis between current rates/ratios and baseline metrics.
- **Simulation Mode Fallback** — Automatic fallback to simulated traffic generation if run without root privileges.
- **CSV & JSON Exporting** — Support for exporting live traffic statistics to CSV and JSON files.

## [0.7.1] - 2026-06-09

### Security
- **CI Dependency Scan Fix** — Upgraded `pip` to `26.1.2` in `uv.lock` to resolve dependency vulnerability `PYSEC-2026-196`.
...
### Fixed
- **Windows CI Unicode Support** — Resolved `UnicodeEncodeError` on Windows CI runners by enabling `PYTHONUTF8: 1` and implementing automatic ASCII sanitization in `BaseWidget` logging.
- **Worker Stability** — Prevented background workers from crashing when attempting to log strings containing non-ASCII symbols (e.g., ✓, 🔄) in restricted terminal environments.
