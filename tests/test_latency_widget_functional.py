...
from src.shared.latency_utils import TracerouteHop, PingStatistics, LatencyStatus
...
async def test_latency_analysis_success():
    ...
    mock_hops = [
        TracerouteHop(hop_number=1, ip_address="192.168.1.1", hostname="router", rtt1_ms=1.0, rtt2_ms=1.1, rtt3_ms=1.2, status="responsive"),
        TracerouteHop(hop_number=2, ip_address="8.8.8.8", hostname="google", rtt1_ms=15.0, rtt2_ms=15.1, rtt3_ms=15.2, status="responsive")
    ]
    ...
    with patch("src.tui.widgets.latency_analyzer_widget.mtr_style_trace", return_value=(mock_hops, "success")), \
